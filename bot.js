...
function extractRichContent(msg) {
  if (msg.photo && msg.photo.length > 0) {
    console.log("RAW caption_entities from Telegram:", JSON.stringify(msg.caption_entities || []));
    const sanitized = sanitizeEntities(msg.caption_entities || []);
    console.log("SANITIZED entities we will store:", JSON.stringify(sanitized));
    return {
      isPhoto: true,
  ...
  console.log("RAW entities from Telegram:", JSON.stringify(msg.entities || []));
  const sanitized = sanitizeEntities(msg.entities || []);
  console.log("SANITIZED entities we will store:", JSON.stringify(sanitized));
  return {
    isPhoto: false,
    fileId: null,
...
// ================================================================
// node-telegram-bot-api has a long-standing, unresolved bug (see GitHub issue
// yagop/node-telegram-bot-api #1240): caption_entities on sendPhoto/sendDocument
// get silently dropped, even though the exact same entities work fine on a plain
// sendMessage. This is why blockquote/premium-emoji formatting was surviving in
// plain-text messages but disappearing specifically on the photo+caption welcome
// message. To keep the single photo+caption experience with full formatting, we
// bypass the library for this one call and hit Telegram's HTTP API directly —
// Telegram itself handles caption_entities correctly, it's only this library's
// internal request-building that mishandles it.
async function sendPhotoWithEntitiesRaw(chatId, fileId, caption, entities, extraOptions = {}) {
  const body = {
    chat_id: chatId,
    photo: fileId,
    caption: caption || "",
  ...
  try {
    ...
    if (!data.ok) {
      console.error("sendPhotoWithEntitiesRaw failed:", JSON.stringify(data));
    }
    return data;
  } catch (e) {
    console.error("sendPhotoWithEntitiesRaw network error:", e.message);
  }
}
...
function sendRichPhoto(chatId, fileId, rawCaption, extraOptions = {}, rawEntities = null) {
  if (rawEntities && rawEntities.length > 0) {
    const entities = sanitizeEntities(rawEntities);
    console.log("SENDING PHOTO caption_entities (raw HTTP):", JSON.stringify(entities));
    return sendPhotoWithEntitiesRaw(chatId, fileId, rawCaption || "", entities, extraOptions);
  }
  const { text, entities } = renderWithEmojis(rawCaption || "");
  if (entities.length > 0) {
    console.log("SENDING PHOTO caption_entities (raw HTTP, :keyword: emoji):", JSON.stringify(entities));
    return sendPhotoWithEntitiesRaw(chatId, fileId, text, entities, extraOptions);
  }
  const options = { ...extraOptions, caption: text, parse_mode: "Markdown" };
