...
// send a photo with a caption that may contain emoji placeholders
function sendRichPhoto(chatId, fileId, rawCaption, extraOptions = {}) {
  const { text, entities } = renderWithEmojis(rawCaption || "");
  const options = { ...extraOptions, caption: text };
  if (entities.length > 0) {
...
// sends the welcome message, with image if one is set
function sendWelcome(chatId, settings, extraOptions = {}) {
  if (settings.welcomeImage) {
    return sendRichPhoto(chatId, settings.welcomeImage, settings.welcomeMessage, extraOptions);
  }
  return sendRich(chatId, settings.welcomeMessage, extraOptions);
}
...
bot.on("message", async (msg) => {
  ...
  if (state.flow === "button") {
    ...
    if (state.step === "value") {
      state.data.value = msg.text.trim();
      const buttons = getButtons();
      const id = state.editId || `btn_${Date.now()}`;
      const newButton = { id, text: state.data.text, type: state.data.type, value: state.data.value };
      const idx = buttons.findIndex((b) => b.id === id);
      if (idx >= 0) buttons[idx] = newButton;
      else buttons.push(newButton);
      saveButtons(buttons);
  ...
  if (state.flow === "setWelcome") {
    ...
    if (msg.photo && msg.photo.length > 0) {
      // use the largest resolution version of the photo
      const fileId = msg.photo[msg.photo.length - 1].file_id;
      settings.welcomeImage = fileId;
      settings.welcomeMessage = msg.caption || "👋 Welcome to " + settings.brandName + "!";
    } else if (msg.text) {
