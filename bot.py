...
EDITABLE_KEYS = {
    ...
    "5": ("explore_button", "Explore Course Button Text"),
}
...
def build_main_markup_json():
    ...
    rows.append([{"text": explore_label, "callback_data": "explore_courses"}])
...
def cb_explore_courses(call):
    ...
    courses = get_courses()
    if not courses:
        bot.send_message(chat_id, "📚 No courses available yet. Check back soon!")
        return

    markup = types.InlineKeyboardMarkup()
    for c in courses:
        markup.row(types.InlineKeyboardButton(c["name"], callback_data=f"viewcourse_{c['id']}"))
    bot.send_message(chat_id, "📚 *Available Courses*\nTap a course to view details:",
                      parse_mode="Markdown", reply_markup=markup)
...
def cb_view_course(call):
    ...
    if not course:
        bot.send_message(chat_id, "⚠️ This course is no longer available.")
        return
